...
class CSV_To_API {
  ...
  function parse_query( $query = null ) {
	...
	$this->fields = isset( $query['fields'] ) ? split(",", $query['fields']) : false;
...
/   * @param string the mixed-ending string to normalized
  ...
  function parse_tsv( $csv ) {
     return $this->parse_csv($csv, "\t");
  }
  /**
   * Turn CSV into a PHP array.
   */
  function parse_csv( $csv, $sep="," ) {

    $csv = $this->normalize_line_endings( $csv );

    $lines = explode( "\n", $csv );
	
    $lines = $this->parse_lines( $lines, $sep );
  ...
  function removeKeys($arr, $fields)
  {
   ...
   */
  function parse_lines( $lines, $sep ) {

    //php 5.3+
    if ( function_exists( 'str_getcsv' ) ) {

      foreach ( $lines as &$line )
        $line = str_getcsv( $line, $sep );
      ...
      while( $line = fgetcsv( $fh, 10000,$sep ) )
        $lines[] = $line;
  ...
  */
  ...
  function commatrim( $str, $rep=1 ) {
	return substr($str, 0, strlen($str)-($rep+1));
  }
  
  function object_to_json2( $data ) {
  	$output .= "{\n";
	$output .= "  \"table\": {\n";
	$output .= "    \"cols\": [\n";
