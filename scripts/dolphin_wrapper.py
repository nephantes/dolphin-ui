...
def getLaneList(run_group_id):
    ...
    sql = "SELECT DISTINCT %s FROM biocore.ngs_runlist nrl, biocore.ngs_runparams nrp, biocore.ngs_samples s, biocore.ngs_temp_lane_files tl where nrl.run_group_id=nrp.run_group_id and nrp.run_status=0 and s.lane_id = tl.lane_id and s.id=nrl.sample_id and nrp.run_group_id='"+str(run_group_id)+"';"
    fields='tl.file_name'

    result=runSQL(sql%fields)
    inputparams=""
    for row in result:
        if (inputparams):
            inputparams=inputparams+":"
