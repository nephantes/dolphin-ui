...
stepIGVTDF='stepIGVTDF%(type)s%(sep)s@RUNIGVTDF -o @OUTDIR -g @GENOMEFASTA -pa @SPAIRED -pu @PUBDIR -w @WKEY -l @TSIZE -sa @SAMTOOLS -t %(type)s -i @IGVTOOLS -se stepIGVTDF%(type)s -j @JGEN%(sep)s5'
stepBam2BW='stepBam2BW%(type)s%(sep)s@RUNBAM2BW -o @OUTDIR -g @GENOMESIZE -p @PUBDIR -wk @WKEY -c @RUNCOVERAGE -t %(type)s -wi @WIGTOBIGWIG -s stepBam2BW%(type)s -j @JGEN%(sep)s5'
stepPicard='stepPicard%(type)s%(sep)s@PERL @RUNPICARD -o @OUTDIR -pu @PUBDIR -w @WKEY -t %(type)s -r @REFFLAT -pi @PICARDCMD -s stepPicard%(type)s -j @JGEN%(sep)s1'
