...
stepRSEMCount='stepRSEMCount%(g_i)s%(t_e)s%(sep)s@RUNRSEMCOUNT -o @OUTDIR -t %(t_e)s -g %(g_i)s -s stepRSEMCount%(g_i)s%(t_e)s -j @JGEN%(sep)s0'
...
stepIGVTDF='stepIGVTDF%(type)s%(sep)s@RUNIGVTDF -o @OUTDIR -g @GENOMEFASTA -l @TSIZE -sa @SAMTOOLS -t %(type)s -i @IGVTOOLS -se stepIGVTDF%(type)s -j @JGEN%(sep)s0'
stepBam2BW='stepBam2BW%(type)s%(sep)s@RUNBAM2BW -o @OUTDIR -g @GENOMESIZE -c @RUNCOVERAGE -t %(type)s -w @WIGTOBIGWIG -s stepBam2BW%(type)s -j @JGEN%(sep)s0'
stepPicard='stepPicard%(sep)s@PERL @RUNPICARD -o @OUTDIR -r @REFFLAT -p @PICARDCMD -s stepPicard -j @JGEN%(sep)s0'
stepCounts = 'stepCounts%(sep)s@RUNCOUNTS -m @MAPNAMES -o @OUTDIR -g @GCOMMONDB -b @MAKEBED -c @BEDTOOLSCMD -s stepCounts -j @JGEN%(sep)s1'
