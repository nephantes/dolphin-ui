...
stepMethylKit = 'stepMethylKit%(methylkit_name)s%(sep)s@RUNMETHYLKIT -sa @COLS%(methylkit_name)s -c @CONDS%(methylkit_name)s -g @GBUILD%(methylkit_name)s -m @MAXCOVERAGE%(methylkit_name)s -to @TOPN%(methylkit_name)s -ste @STEP_SIZE%(methylkit_name)s -ti @TILE_SIZE%(methylkit_name)s -o @OUTDIR -strand @STRAND%(methylkit_name)s -n %(methylkit_name)s -b @BED12FILE -r @RSCRIPT -p @PUBDIR -w @WKEY -se stepMethylKit%(methylkit_name)s -j @JOB%(sep)s30'
