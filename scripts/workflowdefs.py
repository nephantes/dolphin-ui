...
stepMACS='stepMACS%(sep)s@RUNMACS -a @MACSCMD -i @CHIPINPUT -t %(type)s -o @OUTDIR -s stepMacs -p @PREVIOUSPIPE -j @JOB%(sep)s60'
...
stepBSMap='stepBSMap%(sep)s@RUNBSMAP -binpath @BIN/bsmap -digestion @DIGESTION -dspaired @SPAIRED -outdir @OUTDIR -params @BSMAPPARAM -previous @PREVIOUSPIPE -ref @GENOMEFASTA -samtools @SAMTOOLS -servicename stepBSMap -j @JOB%(sep)s30'
stepMCall='stepMCall%(sep)s@RUNMCALL -binpath @BIN/mcall -previous %(type)s -sampleconditions @MCONDS -fields @MFIELDS -params @MCALLPARAM -ref @GENOMEFASTA -o @OUTDIR -servicename stepMCall -j @JOB%(sep)s30'
stepMethylKit = 'stepMethylKit%(methylkit_name)s%(sep)s@RUNMETHYLKIT -sa @MFIELDS -c @MCONDS -g @GBUILD -o @OUTDIR -st @STRAND -n %(methylkit_name)s -t @TILE_SIZE -b @BED12FILE -r @RSCRIPT -p @PUBDIR -w @WKEY -se stepMethylKit%(methylkit_name)s -j @JOB%(sep)s30'
