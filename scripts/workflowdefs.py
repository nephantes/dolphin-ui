...
stepDeeptools='stepDeeptools%(sep)s@RUNDEEPTOOLS -d @DEEPTOOLSHEAT -str @STRANDSPEC -m @MERGEALLSAMPS -k @KMEANS -c @COMPDEEPTOOLS -g @GENOMEBED -p @PLOTTYPE -r @REFTYPE -a @AFTER -bef @BEFORE -l @LENGTHBODY -q @QUALITY -bed @BEDTOOLSINT -t %(type)s -o @OUTDIR -ser stepDeeptools -j @JOB%(sep)s60'
...
stepDiffMeth='stepDiffMeth%(diffmeth_name)s%(sep)s@RUNDIFFMETH -sa @COLS%(diffmeth_name)s -c @CONDS%(diffmeth_name)s -o @OUTDIR -n %(diffmeth_name)s -b @BED12FILE -r @RSCRIPT -p @PUBDIR -w @WKEY %(sep)s30'
