...
stepRSEM = 'stepRSEM%(sep)s@RUNRSEM -cm @RSEMCMD -co @CONVERTRSEM -r %(rsemref)s -d @SPAIRED -g %(genome_bam)s -pub @PUBDIR -w @WKEY -pa @PARAMSRSEM -sa @SAMTOOLS -pr %(previousrsem)s -o @OUTDIR  -ba %(bamsupport)s -bo @BOWTIEPATH -s stepRSEM -j @JOB%(sep)s240'
...
stepPCRDups='stepPCRDups%(type)s%(sep)s@PERL @RUNMERGEPICARD -o @OUTDIR -m @MERGEPDFCMD -p @PUBDIR -w @WKEY -t %(type)s %(sep)s30'
